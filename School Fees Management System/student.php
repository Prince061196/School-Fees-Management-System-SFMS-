...
if (isset($_POST["save"])) {
    ...
    if ($_POST["action"] == "add") {
        ...
        $q1 = $conn->query(
            "INSERT INTO student (sname,joindate,contact,about,emailid,grade,balance,fees) VALUES ('$sname','$joindate','$contact','$about','$emailid','$grade','$balance','$fees')"
        );

        $sid = $conn->insert_id;

        $conn->query(
            "INSERT INTO  fees_transaction (stdid,paid,submitdate,transcation_remark) VALUES ('$sid','$advancefees','$joindate','$remark')"
        );
        ...
        $sql = $conn->query(
            "UPDATE  student  SET  grade  = '$grade', sname = '$sname', contact = '$contact', about = '$about', emailid = '$emailid'  WHERE  id  = '$id'"
        );
...
if (isset($_GET["action"]) && $_GET["action"] == "delete") {
    $conn->query(
        "UPDATE  student set delete_status = '1'  WHERE id='" .
            $_GET["id"] .
            "'"
    );
...
if (isset($_REQUEST["act"]) && @$_REQUEST["act"] == "1") {
    $errormsg =
        "<div class='alert alert-success'> <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Student record has been added!</div>";
} elseif (isset($_REQUEST["act"]) && @$_REQUEST["act"] == "2") {
    $errormsg =
        "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Student record has been updated!</div>";
} elseif (isset($_REQUEST["act"]) && @$_REQUEST["act"] == "3") {
    $errormsg =
        "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Student has been deleted!</div>";
}
?>
            ...
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
						...
						<?php echo (isset($_GET["action"]) && @$_GET["action"] == "add") ||
      @$_GET["action"] == "edit"
          ? ' <a href="student.php" class="btn btn-success btn-sm pull-right" style="border-radius:0%">Go Back </a>'
          : '<a href="student.php?action=add" class="btn btn-danger btn-sm pull-right" style="border-radius:0%"><i class="glyphicon glyphicon-plus"></i> Add New Student</a>'; ?>
						</h1>
               ...
               <div class="panel panel-success">
						...
						<div class="form-group">
								...
								<div class="col-sm-10">
                                    ...
                                    <?php
                                    $sql =
                                        "select * from grade where delete_status='0' order by grade.grade asc";
                                    $q = $conn->query($sql);
						...
						<div class="form-group">
								<label class="col-sm-2 control-label" for="Old">DOR* </label>
								<div class="col-sm-10">
									<input type="text" class="form-control" placeholder="Date of Registration" id="joindate" name="joindate" style="background-color: #fff;" readonly />
								</div>
							</div>
						 </fieldset>
						...
						<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="fees" name="fees" value="<?php echo $fees; ?>" <?php echo $action ==
"update"
    ? "disabled"
    : ""; ?>  />
								</div>
		...
		<div class="panel panel-default">
         ...
         while ($r = $q->fetch_assoc()) {
             echo "<tr " .
                 ...
                 $r["id"] .
                 '" class="btn btn-success btn-xs" style="border-radius:60px;"><span>Edit</span></a>
											
											<a onclick="return confirm(\'Are you sure you want to deactivate this record\');" href="student.php?action=delete&id=' .
                 $r["id"] .
                 '" class="btn btn-danger btn-xs" style="border-radius:60px;"><span>In-Active</span></a> </td>
